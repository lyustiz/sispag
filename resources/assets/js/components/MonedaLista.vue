    ...
    <v-container fluid grid-list-md text-xs-center>
        ...
        <v-card>
            ...
            <template slot="items" slot-scope="item">
                ...
                <td class="text-xs-left">{{ item.item.nb_moneda }}</td>
                <td class="text-xs-left">{{ item.item.co_moneda }}</td>
                <td class="text-xs-left">{{ item.item.tx_signo }}</td>
        ...
        <form-container :nb-accion="nb_accion" :modal="modal" @cerrarModal="cerrarModal">
            <moneda-form :accion="accion" :item="item" @cerrarModal="cerrarModal"></moneda-form>
        </form-container>

        <dialogo 
            :dialogo="dialogo" 
            :mensaje="'Desea Eliminar la Moneda: ' + item.nb_moneda "
            @delItem="delItem"
            @delCancel="delCancel"
        >
        </dialogo>
...
export default {
    ...
    methods:
    {
        ...
        delItem(){
            axios.delete('/api/v1/moneda/'+this.item.id_moneda)
            .then(respuesta => {
